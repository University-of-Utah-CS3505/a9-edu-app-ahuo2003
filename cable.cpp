...
void Cable::mouseMoved(QImage& image, const QPoint &mouseLocation)
{
    ...
    if (mouseLocation.x() < 32 && mouseLocation.x() >= 0 && mouseLocation.y() >= 0 && mouseLocation.y() < 32) {
        if (canDraw){
            ...
            //It will delete the entire path between the end of the cable, and 12 pixels prior if faster deleition is desired,
            //otherwise just the previous pixels.
            if (path.contains(mouseLocation) && (path.indexOf(mouseLocation) >= path.size() - 12)){
...
void Cable::appendToPath(QImage& image, const QPoint &startPoint, const QPoint &endPoint){
    int step = 1;
    if (moveVertical){
...
void Cable::deleteFromPath(QImage &image, const QPoint &startPoint, const QPoint &endPoint)
{
